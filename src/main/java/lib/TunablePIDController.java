...
public class TunablePIDController implements Sendable, AutoCloseable {
  private static int instances;

  // Factor for "proportional" control
  private DoubleSupplier m_kp;

  // Factor for "integral" control
  private DoubleSupplier m_ki;

  // Factor for "derivative" control
  private DoubleSupplier m_kd;
   ...
   */
  public TunablePIDController(DoubleSupplier kp, DoubleSupplier ki, DoubleSupplier kd) {
    this(kp, ki, kd, 0.02);
  }
   ...
   * @param kd The derivative coefficient.
   */
  public void setPID(DoubleSupplier kp, DoubleSupplier ki, DoubleSupplier kd) {
    m_kp = kp;
    m_ki = ki;
    m_kd = kd;
  }
   ...
   */
  public void setP(DoubleSupplier kp) {
    m_kp = kp;
  }
   ...
   */
  public void setI(DoubleSupplier ki) {
    m_ki = ki;
  }
   ...
   */
  public void setD(DoubleSupplier kd) {
    m_kd = kd;
  }
  ...
  public double calculate(double measurement) {
    ...
    if (Math.abs(m_error) > m_iZone) {
      m_totalError = 0;
    } else if (m_ki.getAsDouble() != 0) {
      m_totalError =
          MathUtil.clamp(
              m_totalError + m_error * m_period,
              m_minimumIntegral / m_ki.getAsDouble(),
              m_maximumIntegral / m_ki.getAsDouble());
    }

    return m_kp.getAsDouble() * m_error
        + m_ki.getAsDouble() * m_totalError
        + m_kd.getAsDouble() * m_errorDerivative;
  }
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("p", getP(), null);
    builder.addDoubleProperty("i", getI(), null);
    builder.addDoubleProperty("d", getD(), null);
